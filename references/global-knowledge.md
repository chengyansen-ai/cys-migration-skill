...
本文件沉淀自「Firecrawl 深抽取 22 篇权威指南 + WebSearch 多区域发现（欧美/中东西亚/日韩/南亚/拉美/国内）」。用于把全球提示词工程的共识、跨文化美学、负向词库、平台特性，固化进 `portrait-prompt-master` 技能。
...
> **结论**：本技能的「人物→面容→妆容→身材→装饰→动作→服装→环境→摄像」是上述共识的**国风写实特化版**，方向正确无需改。差异仅在：本技能把"面容/妆容/身材/装饰"抽成可复用的「真实感内核」，并强插"动作迁移约束"。
...
## 二、负向提示词库（ComfyUI / Flux 专用 · 你工作流支持）
...
要点：
- **头大身小专项**：`(head too big:1.4)` + `(distorted proportions:1.3)` 双保险，与正向提示词"头≤25%"呼应。
...
> 用法：你要做「中东/波斯/印度/拉美/日韩」风人像时，取下方签名填进段7（服装）+段8（环境）+段9（摄像氛围）。
...
> 对你的价值：做「战甲/国风/敦煌/边界」题材时，用此法让文化符号**更准更地道**，避免"四不像"。本技能路线 A 的段7/段8 可直接套用此增强。
...
| 平台 | 提示词偏好 | 与你工作流关系 |
|---|---|---|
| **ComfyUI / Flux2 Klein** | Tag 混散文；支持负向；Qwen CLIP 吃中文+括号权重 | **你主线（路线 A）** |
...
> 你业务（参考图→动作迁移→图生视频→抖音）以 ComfyUI/Flux2 为主、GPT Image 2 为辅，故技能默认路线 A 中文 + 路线 B 英文摄影风，已覆盖。
